...
public class EmailUtils {
  ...
  public EmailUtils(String username, String password, String server, EmailFolder emailFolder) throws MessagingException {
    ...
    try {
      props.load(new FileInputStream(new File("src/main/resources/propertyFiles/FrameworkConfiguration.properties")));
    } catch(Exception e) {
      e.printStackTrace();
      System.exit(-1);
    }
  ...
  public void deleteMessages(String subjectToDelete , boolean unreadOnly) throws Exception {
 
  try {
    
      // fetches new messages from server
    Message[] arrayMessages = getMessagesBySubject(subjectToDelete, unreadOnly, getNumberOfMessages());
  ...
  {
    try {
      emailUtils = new EmailUtils("automationggcrackers@gmail.com", "FourThree$#", "smtp.gmail.com", EmailUtils.EmailFolder.INBOX);
      System.out.println("able to login to gmail");
      emailUtils.deleteMessages("GG Crackers | Order Confirmation", true);
      
      Message email = emailUtils.getMessagesBySubject("GG Crackers | Order Confirmation", true, 5)[0];
      System.out.println(email.toString());
     // System.out.println(emailUtils.isTextInMessage(email, "new test"));
      
      if(emailUtils.isTextInMessage(email, "#R996177381"))
        System.out.println("Order is confirmed and Email is recieved to the recipient");
      else 
        System.out.println("Unable to fetch the mail");
      
      if(emailUtils.isTextInMessage(email, "$92.90"))
        System.out.println("Order is confirmed and Price is verified as "+"$92.90");
      else
        System.out.println("price unmatched .............");
   
      System.out.println(emailUtils.getMessageContent(email));
     // Assert.assertTrue("Approval message is not in email", emailUtils.isTextInMessage(email, "You have been approved"));
    
    } catch (IndexOutOfBoundsException e) {
      System.out.println("No Unread Messages found .... Mailer failure ");
